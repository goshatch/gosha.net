...
Here is the way I’ve setup my [Doom Emacs](https://github.com/doomemacs/doomemacs) for studying this book, experimenting with code, and taking notes (with `org-mode`). I had to piece together this setup from various sources online, this is what’s worked for me.
...
Install Racket via Homebrew. I chose the `minimal-racket` formula, as prefer to write code in Emacs and did not want to install [DrRacket](https://docs.racket-lang.org/drracket/index.html) (an IDE).
...
I’m using Doom Emacs, so these steps assume you do too. If you’re using you own config, I would recommend that you check out Konstantinos Chousos’s [SICP in Emacs](https://kchousos.github.io/posts/sicp-in-emacs/) post, which also has some useful additional info, such as how to read the book in the Texinfo format.
